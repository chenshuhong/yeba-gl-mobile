<template>
  ...
  <scroll
    ...
    <group slot="top">
      <router-link to="/horse/HorseConditionAdd">
    ...
    <group v-for="item in list" :key="item.id">
      <cell :title="item.conditionDesc">
        ...
        <router-link
          :to="{ name: 'HorseConditionAdd', params: item}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPrizesList () {
        this.onloadStart()
        this.$api.getGamesPrizesList(this.$barId, 4).then((data) => {
          let list = []
...
//      删除奖品
      ...
      onEventBus (data) {
        if (data.type === 'horseConditionEdit') {
          this.$util.change1(this.list, this.changeData(data.data))
        } else if (data.type === 'horseConditionAdd') (
