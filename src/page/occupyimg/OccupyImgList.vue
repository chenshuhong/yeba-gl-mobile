<template>
    ...
    <scroll
      ...
      @pullingDown="getOccupyImgList"
      @reLoadData="getOccupyImgList">
      <group slot="top">
        <router-link to="/occupyimg/OccupyImgAdd">
      ...
      <group v-for="(item,key) in list" :key="item.id">
        <cell :title="'霸屏套餐'+(key+1)">
          <div class="switch-container">
            ...
            <router-link
              :to="{ name: 'OccupyImgAdd', params: item}">
...
//      获取列表
      getOccupyImgList() {
        this.onloadStart()
        this.$api.getOccupyImg(this.$barId).then((data) => {
          let list = []
          let batchData = []
...
//      根据id删除霸屏套餐
      ...
      onEventBus(data){
        if (data.type === 'occupyImgEdit') {
          this.$util.change1(this.list, this.changeData(data.data))
        } else if (data.type === 'occupyImgAdd') (
