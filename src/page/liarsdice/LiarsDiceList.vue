<template>
  ...
  <scroll
    ...
    <group slot="top">
      <router-link to="/liarsdice/LiarsDicePrizeSet">
        <span class="my-Add">添加条件</span>
      </router-link>
    </group>
    <group v-for="item in list" :key="item.id">
      <cell :title="item.conditionDesc">
        ...
        <router-link
          :to="{ name: 'LiarsDicePrizeSet', params: item}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPrizesList () {
        this.onloadStart()
        this.$api.getDicePrize(this.$barId).then((data) => {
          let list = []
...
//      删除奖品
      deletePrize (id) {
        this.$api.deleteDicePrize(this.$barId, id).then((res) => {
          this.$util.remove(this.list, id)
        })
      },
      onEventBus (data) {
        if (data.type === 'diceConditionEdit') {
          this.$util.change1(this.list, this.changeData(data.data))
        } else if (data.type === 'diceConditionAdd') (
