...
//      添加
      addPrize (e) {
        if (e) {
          ...
          if (params.id) {
            this.$api.editDicePrize(this.$barId, params.id, data).then((item) => {
              this.$store.commit(this.$config.STORAGE.EVENT_BUS, {
                type: 'diceConditionEdit',
                data: item
              })
              this.$router.back()
            })
          } else {
            this.$api.addDicePrize(this.$barId, data).then((item) => {
              this.$store.commit(this.$config.STORAGE.EVENT_BUS, {
                type: 'diceConditionAdd',
                data: item
              })
              this.$router.back()
            })
